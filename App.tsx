...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col font-sans text-slate-100">
      ...
      {/* Top Navigation Bar */}
      <nav className="h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-center gap-2 md:gap-8 shadow-lg z-50">
         <NavButton 
           label="Sokoban" 
...
const NavButton = ({ label, active, onClick, icon }: { label: string, active: boolean, onClick: () => void, icon: string }) => (
  <button
    onClick={onClick}
    className={`
      flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-200 font-bold text-sm md:text-base
      ${active 
